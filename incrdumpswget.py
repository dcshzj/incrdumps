...
def upload(wiki):
	global count, filelist, tempdir, userdate
	...
	for thefile in filelist:
		...
		if (count == 0):
			curl = ['curl', '--retry 20', '--location',
					...
					'--header', "'x-archive-meta-title:Wikimedia incremental dump files for %s on %s'" % (wiki, userdate),
					'--header', "'x-archive-meta-description:This is the incremental dump files for %s that is generated by Wikimedia on %s.'" % (wiki, userdate),
