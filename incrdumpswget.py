...
def downloaddump(wiki):
	...
	for thefile in filelist:
		...
		if (curfile.startswith('-')):
			...
			thedumpfile = curfile
		os.system('wget ' + wgethost + '/' + wiki + '/' + userdate + '/' + thedumpfile)

def upload(wiki):
	global count, filelist, tempdir, curdate, langname, sitename, userdate
	os.chdir(tempdir + '/' + wiki)
	for thefile in filelist:
		curfile = ''.join(thefile)
		if (curfile.startswith('-')):
			thedumpfile = wiki + '-' + userdate + curfile
		...
		if (count == 0):
			curl = ['curl', '--retry 3', '--location',
					...
					'--header', "'x-archive-meta-title:Wikimedia incremental dump files for %s%s on %s'" % (langname, sitename, curdate),
					'--header', "'x-archive-meta-description:This is the incremental dump files for %s%s that is generated by Wikimedia on %s.'" % (langname, sitename, curdate),
